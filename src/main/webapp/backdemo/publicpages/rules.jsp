    ...
    <div class="container-fluid">
        ...
        <div class="content-right">
            <div class="header">
                ...
                <div class="new-body">
                    <form action="../../insertNews.mvc" method="POST" role="form" id="form">
                        <c:if test="${singleNews != null}">
                            <input type="hidden" name="newsId" value="${singleNews.newsId}">
                        </c:if>
                        <div class="form-group">
                            <label for="">制度标题</label>
                            <input type="text" name="title" class="form-control" id="" placeholder="请输入标题" value="${singleNews.title}">
                        </div>
                        <div class="form-group">
                            <label for="exampleInputFile">上传附件</label>
                            <input type="file" name="paths" id="exampleInputFile">
                        </div>
                        <input type="hidden" value="9" name="classId">
                    ...
                    <c:if test="${singleNews == null}">
                        <div class="show-btn">
                            <button class="btn btn-primary" onclick="submitRule()">发布</button>
                        </div>
                    </c:if>
                    <div class="show-btn">
                        <c:if test="${singleNews != null}">
                ...
                <div class="show-news">
                    <form class="navbar-form" action="../../selectRulesWithName.mvc">
                        <div class="form-group">
                    ...
                    <table class="table table-hover table-striped">
                        ...
                        <c:forEach items="${news}" var="news" varStatus="id">
                            <c:if test="${news.classify.classifyId == 9}">
                                <tr>
                                    <td>${id.count}</td>
                                    <td>${news.title}</td>
                                    <td>${news.time}</td>
...
<script>
    ...
    function deleteWork(index) {
        $.ajax({
            url: "<%=basePath%>deleteNews.mvc?index="+index,
            type: "post",
            success: function (data) {
                alert("删除成功!")
                window.location.reload();
    ...
    function uploadWork(index) {
        $.ajax({
            url: "<%=basePath%>uploadNews.mvc?index="+index,
            type: "post",
            success: function (data) {
                window.location.reload();
            },
